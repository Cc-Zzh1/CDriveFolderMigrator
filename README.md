...
- 也可用于：Windows Server 2016/2019/2022 等带桌面体验的系统。
- 需要：Windows PowerShell 5.1、.NET Framework WinForms、`robocopy`、`mklink`，这些组件在现代 Windows 中通常自带。
- 不支持：macOS、Linux。
- 不建议：Windows 7/8/8.1 等已停止支持的旧系统，即使部分环境可能可以运行，也没有做兼容保证。

迁移当前用户目录下的游戏存档、浏览器数据和软件配置时，通常不需要管理员权限。不要用它迁移系统目录、驱动仓库或其他需要管理员权限维护的目录。
...
还原成功后，工具会检查原路径旁边是否存在自己生成的临时备份，例如：

```text
Example_backup_migrated_20260706_010000
```

如果检测到这类备份，工具会同步删除它，避免 C 盘继续保留旧副本。工具只会删除符合 `_backup_migrated_yyyyMMdd_HHmmss` 命名规则、并且位于原路径同级目录下的备份；如果备份被占用导致删除失败，日志会提示你手动检查。
